...
import { getCustomerActivity, getCartItems, getRecommendations } from '../services/api'
...
const CustomerActivity = ({ customer }) => {
  ...
  const fetchData = async () => {
    try {
      setLoading(true)
      const [activityData, cartData, recommendationsData] = await Promise.all([
        getCustomerActivity(),
        getCartItems(),
        getRecommendations()
      ])
      ...
      // Mock data for demo
      setActivity({
        totalSpent: customer?.total_spent || 2500,
        totalOrders: customer?.total_orders || 5,
        lastPurchase: customer?.last_purchase_date || '2024-04-05',
        favoriteCategory: 'Electronics',
        loyaltyPoints: 1250
      })
      setCartItems([
        { id: 1, name: 'iPhone 15 Pro', price: 79999, image: '📱', quantity: 1 },
        { id: 2, name: 'AirPods Pro', price: 24999, image: '🎧', quantity: 1 }
      ])
      setRecommendations([
